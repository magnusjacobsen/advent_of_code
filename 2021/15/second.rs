...
fn main() {
    ...
    let input = std::io::stdin().lines().map(|x| x.unwrap().chars().map(|y| y.to_digit(10).unwrap() as usize).collect::<Vec<_>>()).collect::<Vec<_>>();
    let og_width = input.len();
    let width = og_width * 5;
    let mut risk = vec![vec![0; width]; width];
    for i in 0..width {
        for j in 0..width {
            let to_add = i / og_width + j / og_width;
            let v = (input[i % og_width][j % og_width] + to_add) % 9;
            risk[i][j] = if v == 0 { 9 } else { v };
        }
    }
